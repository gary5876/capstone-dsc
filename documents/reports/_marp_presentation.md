...
size: 16:9
paginate: true
header: 'DSC 검증 캡스톤 · 2026-04-25'
style: |
  section { font-family: 'Malgun Gothic', 'Apple SD Gothic Neo', sans-serif; padding: 40px 50px; }
  ...
  table { font-size: 0.8em; margin: 6px 0; }
...
### 평가
- 학습 405회 후 (DSC, F1) 405쌍의 Pearson 상관계수
- 가설 입증 기준: r ≥ 0.3, p < 0.001

---

<!-- _slide 3: 1차 결과: 가설 입증 실패 -->
...
| | v2 | **v3.2 정식** | 변화 |
|---|---:|---:|---|
| r | 0.085 | **0.429** | **+0.34** (5배) |
| p-value | 0.147 (비유의) | **1.50e-19** (강한 유의) | 18 자릿수 개선 |
| n | 295 | **405** | 강도 0.9·0.95 추가 |

→ **가설 입증 기준(r≥0.3, p<0.001) 모두 통과.**

---

<!-- _slide 9: 정식 결과 2: 부호 반전 회복 -->
...
<!-- _slide 10: 정식 결과 3: 등급 비교 가능성 회복 -->
...
ANOVA: **F = 39.16, p = 3.2e-22** — 등급 간 F1 차이 매우 강한 유의성
→ DSC 등급이 모델 성능을 단조적으로 예측

---

<!-- _slide 11: 잔여 한계 & 정직한 보고 -->
...
### 3. letter × uniqueness 6단계 학습 누락
- letter 16k행 → uniqueness 폴루션 시 80~112k행으로 확장
- 노트북 03에서 timeout 추정 → 재학습 필요
...
### 캡스톤 가설 입증
**DSC 점수가 ML 성능을 통계적으로 예측한다 (Pearson r = 0.429, p = 1.5e-19, n = 405)**
**ANOVA로 등급별 F1 차이도 강한 유의성 (p = 3e-22)**
...
### 차기 계획
- letter × uniqueness 재학습으로 411 → 411건 보완
- v3.3에서 uniqueness 가중치 추가 조정 검토
- DQ4AI 외 polluter (예: drift over time) 추가 실험

---

<!-- _slide 13: Q&A 대비 보조 자료 -->
...
| 정식 모델 성능 | `results/model_performance.csv` (405건) |
